...
module.exports = class extends BaseGenerator {
    constructor(args, opts) {
        ...
        if (blueprints && blueprints.trim().length > 0) {
        ...
        this.info(`Using blueprints: ${this.blueprints}`);
        ...
        return {
            ...
            setupconsts() {
                ...
                if (this.blueprints && this.blueprints.length > 0) {
                    ...
                    this.otherModules = this.otherModules.filter(module => this.blueprints.findIndex(bp => bp === module.name) === -1);
                    this.otherModules.push(
                        ...this.blueprints.map(bp => {
                            return { name: bp, version: this.findBlueprintVersion(bp) };
                        })
                    );
                }
                            ...
                            this.gitExec('add -A', { trace: false }, () => {
                                ...
                                if (this.blueprints && this.blueprints.length > 0) {
                                    const bpInfo = this.blueprints
                                        .map(
                                            blueprint =>
                                                `${blueprint.replace('generator-jhipster-', '')}-${this.findBlueprintVersion(blueprint) ||
                                                    'latest'}`
                                        )
                                        .join(', ');
